    ...
    epsilon = 1e-06, maxit = 100, df=Inf)
{
    ...
    Sigma0 <- Sigma
    Sigmaord <- Sigma
    Sigmaord <- contord(marginal, Sigma, support, Spearman, df=df)
    Sigmaold <- Sigma
    Sigmaordold <- Sigmaord
        ...
        for (q in 1:(k - 1)) {
			...
			while (max(abs(Sigmaord[q, r] - Sigma0[q, r]) > epsilon) & it < maxit) {
                ...
                Sigma[r, q] <- Sigma[q, r]
      	    Sigmaord[r, q] <- contord(list(marginal[[q]],marginal[[r]]), matrix(c(1,Sigma[q, r],Sigma[q, r],1),2,2), list(support[[q]],support[[r]]), Spearman, df=df)[2]
